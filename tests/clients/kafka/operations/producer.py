...
class OperationsKafkaProducerTestClient(KafkaProducerTestClient):
    ...
    def produce_operation_event_api(self, event: OperationEventTestSchema) -> OperationEventTestSchema:
        self.produce(topic='operations-service.operation-event.inbox', value=event.model_dump_json())
        self.flush_all()
